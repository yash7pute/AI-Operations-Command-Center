...
import '@jest/globals';
import type { FeedbackRecord, FeedbackOutcome, FeedbackStats } from '../../src/agents/learning/feedback-tracker';
...
import type { PromptExample, PromptTemplate } from '../../src/agents/learning/prompt-optimizer';
...
describe('Learning System Tests', () => {
    ...
    describe('Feedback Tracking', () => {
        ...
        test('should record modification feedback and learn from changes', () => {
            ...
            // System should learn: "meeting request" â†’ schedule_meeting action
    ...
    describe('Prompt Optimization', () => {
        ...
        test('should perform A/B test and select winner', () => {
            const variantA: PromptTemplate = {
                ...
                version: 1,
                type: 'classification',
                systemPrompt: 'Classify signals - Version A',
                examples: [],
                maxExamples: 10,
                exampleFormat: 'Input: {input}\nOutput: {output}',
            ...
            const variantB = createPromptTemplate({ ... });
    ...
    describe('Performance Degradation Prevention', () => {
        ...
        test('should rollback if new prompt performs worse', () => {
            ...
            const shouldRollback = 
                newTemplate.metadata.performanceMetrics.successRate < 
                oldTemplate.metadata.performanceMetrics.successRate - 0.10; // 10% threshold
            
            expect(shouldRollback).toBe(true);
            
            if (shouldRollback) {
                // Use old template
                expect(oldTemplate.metadata.performanceMetrics.successRate).toBe(0.85);
            }
        });
...
afterAll(() => {
    ...
    console.log('\nAll tests validate:');
    console.log('  âœ“ Feedback recording (positive, negative, modifications)');
    console.log('  âœ“ Pattern detection after 10+ similar signals');
    console.log('  âœ“ Pattern application to new signals');
    console.log('  âœ“ Anomaly detection (unusual signals)');
    console.log('  âœ“ Prompt optimization (add/remove examples)');
    console.log('  âœ“ A/B testing for prompt improvements');
    console.log('  âœ“ Learning data persistence');
    console.log('  âœ“ Performance monitoring and degradation prevention');
